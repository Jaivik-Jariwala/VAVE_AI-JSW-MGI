...
import pandas as pd
import numpy as np
from flask import Flask, request, jsonify, send_file, url_for, render_template, redirect, flash, session
...
def get_db_connection():
    ...
    try:
        conn = psycopg2.connect(
            host=os.getenv('DB_HOST'),       # <-- Correct: Use the KEY
            port=os.getenv('DB_PORT'),       # <-- Correct: Use the KEY
            dbname=os.getenv('DB_NAME'),     # <-- Correct: Use the KEY
            user=os.getenv('DB_USER'),       # <-- Correct: Use the KEY
            password=os.getenv('DB_PASS')    # <-- Correct: Use the KEY
        )
        return conn
    except Exception as e:
    ...
    try:
        if image_path:
            ...
            if not table_data:
                ...
                csv_filename = generate_csv_from_table([])
                ppt_filename = generate_ppt_from_response("", [])
                return {
...
"""
                ...
                for i in range(max_tokens):
                    try:
                        ...
                        if generated.size(1) > 2048:
                            logger.warning("Generation stopped: max sequence length")
                            break
                    except Exception as e:
        ...
        return {
            ...
            "table_data": [], "image_urls": [], "csv_url": "", "ppt_url": "",
...
def chat_app():
    ...
    # Pass the username to the template if needed
    return render_template('chat_app.html', username=current_user.username)
