document.addEventListener('DOMContentLoaded', () => {
    ...
    chatBtn.addEventListener('click', async function() {
        ...
        try {
            ...
            // Store data for on-demand generation
            window.currentTableData = data.table_data || [];
            window.currentResponseText = data.response_text || '';
            
            // Show download buttons if we have data
            if (data.table_data && data.table_data.length > 0) {
                html += `<p>
                    <button id="downloadExcelBtnScript" class="download-link" style="margin-right: 10px;">Download Excel</button>
                    <button id="downloadPptBtnScript" class="download-link">Download PPT</button>
                </p>`;
            }
            if (data.audio) {
            ...
            const excelBtn = document.getElementById('downloadExcelBtnScript');
            const pptBtn = document.getElementById('downloadPptBtnScript');
                ...
                excelBtn.addEventListener('click', async function() {
                    ...
                    try {
                        const response = await fetch('/generate_excel', {
                            ...
                            body: JSON.stringify({ table_data: window.currentTableData })
                        });
                    ...
                    try {
                        const response = await fetch('/generate_ppt', {
                            ...
                            body: JSON.stringify({ 
                                ...
                                response_text: window.currentResponseText 
                            })
                        });
                        ...
                        if (result.success && result.download_url) {
                            window.location.href = result.download_url;
                            showNotification('PPT file generated successfully', 'success');
                        } else {
